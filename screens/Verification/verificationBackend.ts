...
export const verifyCode = async (
  ...
  try {
    const response = await fetch('http://192.168.0.18:8000/api/auth/verify-verification-code', {
      method: 'PATCH',
      headers: {
...
export const resendVerificationCode = async (
  ...
  try {
    const response = await fetch('http://192.168.0.18:8000/api/auth/send-verification-code', {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
      },
