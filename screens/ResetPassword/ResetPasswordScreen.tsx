...
export default function ResetPasswordScreen() {
  ...
  const handleValidateCode = async () => {
    try {
      const response = await axios.post("http://172.16.6.156:8000/api/auth/validate-forgot-password-code", {
        email,
        providedCode: code,
      })
  ...
  const handleResetPassword = async () => {
    try {
      await axios.patch("http://172.16.6.156:8000/api/auth/verify-forgot-password-code", {
        email,
        providedCode: code,
        newPassword,
      })
